{{-- resources/views/wm/dashboard.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Веб-мастер: дашборд</h2>
    </x-slot>
    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="card"><div class="card-body">
                <div class="text-sm text-gray-500">Мои клики (всего)</div>
                <div class="text-2xl font-semibold tabular-nums">{{ number_format($stats['clicks_all'] ?? 0, 0, '.', ' ') }}</div>
            </div></div>
            <div class="card"><div class="card-body">
                <div class="text-sm text-gray-500">Мои клики (валидные)</div>
                <div class="text-2xl font-semibold tabular-nums">{{ number_format($stats['clicks_valid'] ?? 0, 0, '.', ' ') }}</div>
            </div></div>
            <div class="card"><div class="card-body">
                <div class="text-sm text-gray-500">Ожидаемая выплата (₽)</div>
                <div class="text-2xl font-semibold tabular-nums">{{ number_format($stats['payout_rub'] ?? 0, 2, '.', ' ') }}</div>
            </div></div>
            <div class="card"><div class="card-body">
                <div class="text-sm text-gray-500">Активных подписок</div>
        ...
        <div class="mt-6 overflow-x-auto bg-white border rounded">
            <table class="w-full text-left">
                ...
                @empty
                    <tr><td class="p-4 text-gray-600" colspan="6">Нет кликов.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
