{{-- resources/views/adv/offers/create.blade.php --}}
<x-app-layout>
    <x-slot name="header">
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white border rounded p-6">
                <form method="POST" action="{{ route('adv.offers.store') }}" class="space-y-5">
                    @csrf
                    ...
                    {{-- CPC --}}
                    <div>
                        <label class="block text-sm text-gray-700 mb-1" for="cpc">Стоиомсть клика (₽)</label>
                            ...
                            @foreach($topics as $t)
                                <label class="inline-flex items-center gap-2">
                                    <input type="checkbox" name="topics[]" value="{{ $t->id }}"
                                           class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                                           {{ in_array($t->id, old('topics', []), true) ? 'checked' : '' }}>
                                    <span>{{ $t->name }}</span>
                                </label>
                            @endforeach
                        </div>
